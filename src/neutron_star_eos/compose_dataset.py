...
class ComposeSliceReport:
    ...
    def continuous_barotrope_available(self) -> bool:
        return (
            ...
            and not any(item.severity == "barotrope_blocker" for item in self.diagnostics)
        )
...
class ComposeColdSlice:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            or any(right <= left for left, right in zip(resolved_positions, resolved_positions[1:]))
        ):
            raise EosInputError(
                "CompOSE source positions must align with rows and increase strictly"
            )
        self.source_positions = resolved_positions
        density_by_index = dict(zip(dataset.baryon_density.indices, dataset.baryon_density.values))
    ...
    def report(self) -> ComposeSliceReport:
        ...
        euler_residual = pressure - (self._baryon_density * chemical_potential - epsilon)
        scale = np.maximum.reduce(
            (np.abs(pressure), np.abs(self._baryon_density * chemical_potential), np.abs(epsilon))
        )
        if np.any(scale <= 0.0) or np.any(~np.isfinite(scale)):
            raise EosInputError("selected CompOSE path has an invalid Euler normalization")
    ...
    def selected_domain(
        ...
        *,
        ...
        lower = -np.inf if baryon_density_min_fm3 is None else float(baryon_density_min_fm3)
        upper = np.inf if baryon_density_max_fm3 is None else float(baryon_density_max_fm3)
        ...
        if upper <= 0.0 or upper <= lower:
            raise EosInputError("selected baryon-density limits must satisfy 0 < min < max")
        ...
        if np.any(np.diff(positions) != 1):
            raise EosInputError("selected CompOSE density domain must be one contiguous source block")
    ...
    def diagnostic_monotone_subsequence(
        ...
        *,
        ...
        for position in range(1, len(self.rows)):
            previous = retained[-1]
            if pressure[position] > pressure[previous] and epsilon[position] > epsilon[previous]:
        ...
        return ComposeColdSlice(
            ...
            phase_codes=phase_codes,
            matter_declaration=self.matter_declaration,
            source_positions=tuple(self.source_positions[position] for position in retained),
        )
    ...
    def continuous_segments(self) -> tuple[tuple[int, int], ...]:
        ...
        split_after = sorted(
            {
                item.left_position
                for item in (*report.pressure_ordering_issues, *report.energy_density_ordering_issues)
            }
        )
    ...
    def cold_beta_equilibrium_slice(
        ...
        *,
        ...
        if includes_leptons is not True:
            raise EosInputError("cold stellar reduction must explicitly include leptons")
...
def load_compose_dataset(
    ...
    *,
    ...
    if any(value <= 0.0 for value in density.values):
        raise EosInputError("CompOSE baryon-density axis eos.nb must be strictly positive")
