    ...
    *,
    ...
    if values is None:
        ...
        if query.ndim != 1 or not len(query):
            raise ValueError("baryon_density_fm3 must be a non-empty one-dimensional array")
        if np.any(~np.isfinite(query)) or np.any(np.diff(query) <= 0.0):
            raise ValueError("baryon_density_fm3 must be finite and strictly increasing")
...
def _linear_fields(
    ...
    values = source_values[intervals] + (
        query - nodes[intervals]
    )[:, None] * selected_slopes
    return values, selected_slopes


def _parse_composition_payload(tokens: tuple[str, ...]) -> tuple[dict[int, float], dict[int, tuple[float, float, float]]]:
    try:
...
def _parse_microphysics(cold_slice: ComposeColdSlice) -> dict[tuple[int, int, int], dict[int, float]]:
    if "eos.micro" not in cold_slice.dataset.available_files:
        return {}
    try:
        lines = cold_slice.dataset.source_file_bytes("eos.micro").decode("ascii").splitlines()
...
def _optional_source_fields(
    ...
    for row in rows:
        ...
        parsed_composition.append(
            ({}, {}) if source is None else _parse_composition_payload(source.raw_payload_tokens)
        )
    pair_codes = sorted({code for pairs, _quads in parsed_composition for code in pairs})
    for code in pair_codes:
        ...
        descriptions[name] = f"CompOSE composition pair quantity for particle code {code}"
    ...
    micro_codes = sorted(
        {code for row in rows for code in micro_map.get(row.key, {})}
    )
    ...
    if cold_slice.phase_codes is not None:
        ...
        descriptions["phase_code"] = "Model-specific CompOSE phase code; not interpreted physically"
...
def _sample_optional_field(
    ...
    *,
    ...
    if piecewise_constant:
        ...
        fraction = (query - nodes[intervals]) / (nodes[intervals + 1] - nodes[intervals])
...
def interpolate_compose_thermodynamics(
    ...
    *,
    ...
    if len(nodes) < 2 or source_q.shape != (len(nodes), 7):
        raise EosInputError("native CompOSE thermodynamics requires at least two seven-Q rows")
    query = _query_grid(
        nodes,
        baryon_density_fm3,
        points=int(points),
    ...
    columns: dict[str, np.ndarray] = {
        ...
        "gibbs_free_energy_per_baryon_over_mn_minus_1": gibbs_per_baryon / neutron_mass - 1.0,
    ...
    optional, optional_units, optional_descriptions = _optional_source_fields(cold_slice)
    ...
    exact = np.zeros(len(nodes), dtype=bool)
    exact[valid_positions] = query[query_positions[valid_positions]] == nodes[valid_positions]
